...
export function editAssignedMeter(
  { commit, state },
  { meter, services, generation }
) {
  return new Promise((resolve, reject) => {
    meters
      .updateDesignatedMeter({ meter, services, generation })
      .then((res) => {
        meter.services = res;
...
export function getCurrentCfePeriod({ commit, state }, company) {
  adminValues
    .findByDate(state.cfeValues.currentDate, company.Division)
    .then(({ cfeValue }) => {
      commit(
        mutation.GET_CURRENT_CFE_VALUES,
        Object.assign({}, cfeValue[company.tariff_type])
      );
    })
    .catch((err) => {
      console.log(err);
...
export function changeCfePeriod({ commit, state }, { date, division }) {
  let new_date = moment(state.cfeValues.date)
    .add(date.years, "Y")
    .add(date.months, "M")
    .format();
  adminValues
    .findByDate(new_date, division.text)
    .then(({ cfeValue }) => {
...
export function setCfePrices({ commit, state }, { payload, city, tariffType }) {
  return new Promise((resolve, reject) => {
    adminValues
      .createOrUpdatePrices(state.cfeValues.date, city, payload, tariffType)
      .then(({ cfeValue }) => {
        console.log("entre x2");
        console.log(cfeValue);
        commit(
          mutation.GET_CFE_VALUES,
          Object.assign({ new_date: cfeValue.date }, cfeValue, {
