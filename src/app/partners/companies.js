...
export default {
    ...
    computed: {
        ...
        divisiones() {
            return Constants
                .Divisiones
                .map((divisiones, index) => ({ value: index, text: divisiones }));
        },
        cities() {
            return Constants
                .Cities
                .map((city, index) => ({ value: index, text: city }));
        },
        tarifas() {
            return Constants
                .tarifas
                .map((tarifa, index) => ({ value: index, text: tarifa }))
        }
    ...
    methods: {
        ...
        createCompany() {
            if (!this.verifyData()) {
                this.$notify(
                    { group: 'notification', type: 'warn', text: 'Verifica que los datos estén llenados de la mejor manera' }
                );
                return;
            }
            ...
            companies
                ...
                .then(newCompany => {
                    ...
                    this.resetCompany();
                    this.$notify(
                        { group: 'notification', type: 'success', text: 'La compañía se ha creado con éxito' }
                    );

                })
                .catch(() => {
                    this.$notify(
                        { group: 'notification', type: 'error', text: 'Hubo un error al crear la compañía' }
                    );
                });
            companies.find({
                filter: {
                ...
                companies
                    ...
                    .then(compañias => {
                        ...
                        companies
                            .PATCH(this.$store.state.user.user.company_id, { "Administrando": empresasids })
                            .then(res => { })
                    });
            })
        ...
        addNewCompanyUsers(newCompany) {
            ...
            companies
                ...
                .then(() => {
                    this.$notify(
                        { group: 'notification', type: 'success', text: 'Usuarios creados con éxito' }
                    );
                })
                .catch(() => {
                    this.$notify(
                        { group: 'notification', type: 'error', text: 'Hubo un error al crear usuarios' }
                    );
                });
        },
        addCompany(company) {
